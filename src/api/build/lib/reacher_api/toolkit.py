...
class REACHER:
    ...
    def start_program(self):
        ...
        self.program_start_time = time.time() # FIXME: format to readable time   
...
if __name__ == "__main__":
    ...
    # Simulate setting a COM port
    try:
        test_port = "COM3"  # Change to an actual port on your machine
        reacher.set_COM_port(test_port)
        print(f"Set COM port to: {test_port}")
    except Exception as e:
        print(f"Error setting COM port: {e}")

    # Test opening and closing the serial connection
    try:
