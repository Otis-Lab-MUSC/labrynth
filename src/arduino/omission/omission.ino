#define SKETCH_NAME "omission.ino"
...
unsigned long lastInfusionTime = 0;        // time of the last infusion
unsigned long omissionInterval = 20000;   // time necessary of no presses before infusion delivery
...
// ---------- Utility functions (functions that perform one iteration upon instance call) ----------
...
void triggerInfusion()
{
  /*
     ...
     Triggers the pump for an infusion.
  ...
  */
  ...
  if (programIsRunning && (currentMillis - lastInfusionTime >= omissionInterval))
  {
    if (pump.isArmed() && cs.isArmed())
    {
      ...
      cs.setOnTimestamp(timestamp); // set the cue tone period
    ...
    { // if the debounce period has passed, the lever's state is stable
      if (currentLeverState != lever->getStableLeverState())
      {
        ...
        { // lever press detected (assuming LOW means pressed)
          ...
          definePressActivity(programRunning, lever); // determine whether the press is active
          if (lever->getPressType() == "ACTIVE")
          {
            lastInfusionTime = millis(); // reset omission timer upon active press
          }
        }
        else
        { // lever release detected
          lever->setReleaseTimestamp(millis()); // set the release timestamp at the moment of detection
          pressingDataEntry(lever);       // handle data entry on lever release
        }
      }
    }
...
void monitorSerialCommands()
{
  ...
  { // checks to see if there is an input from serial
    ...
    {
      long initOmissionInterval = command.substring(String("SET_OMISSION_INTERVAL:").length()).toInt();
...
void PROGRAM()
{
  ...
  */
  if (linkedToGUI)
  {
    monitorPressing(programIsRunning, activeLever);         // monitor pressing for active lever requires cs and pump parameter
    monitorPressing(programIsRunning, inactiveLever); // monitor pressing for inactive lever does not require cs or pump parameter
