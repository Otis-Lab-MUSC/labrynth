...
/** Fixed time window: always show exactly 1 second of stimulation. */
...
export function SquareWaveCanvas({ frequency, duration }: SquareWaveCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Read theme colors fresh each draw.
    const accent = readCssColor("--color-accent");
    const textColor = readCssColor("--color-text-primary");
    const borderColor = readCssColor("--color-border");
    const panelColor = readCssColor("--color-panel");
    ...
    const gridStep = niceGridInterval(TIME_WINDOW, 5);
    const firstGrid = Math.ceil(tStart / gridStep) * gridStep;
    for (let t = firstGrid; t <= tEnd; t += gridStep) {
      const x = PADDING_LEFT + ((t - tStart) / TIME_WINDOW) * plotW;
      ctx.strokeStyle = rgb(borderColor, 0.15);
      ctx.beginPath();
    ...
    for (let t = firstGrid; t <= tEnd; t += gridStep) {
      const x = PADDING_LEFT + ((t - tStart) / TIME_WINDOW) * plotW;
      ctx.fillText(formatTimeLabel(t, TIME_WINDOW), x, PADDING_TOP + plotH + 4);
    }
    // Label the left edge if not near a grid line.
    if (firstGrid - tStart > gridStep * 0.3) {
      ctx.fillText(
        formatTimeLabel(tStart, TIME_WINDOW),
        PADDING_LEFT,
        PADDING_TOP + plotH + 4,
      );
    }
    // X-axis unit label.
    ctx.fillText(timeUnit(TIME_WINDOW), PADDING_LEFT + plotW + 6, PADDING_TOP + plotH + 4);
    ...
    if (tStart >= durationSec) {
      ...
      if (durationSec < tEnd) {
        const xBurst = PADDING_LEFT + ((durationSec - tStart) / TIME_WINDOW) * plotW;
        ctx.lineTo(xBurst, y);
        if (y !== yLow) {
