      ...
      },
      {
        ...
        subsections: [
          ...
          },
          {
            ...
            title: "Levers",
            content:
              "Two levers (RH and LH) can be configured independently. Parameters:\n\n- Timeout: duration (ms) of the timeout period after a reinforced press\n- Timeout starts on: which presses open that timeout window\n\nPress types: Active (counts toward the reinforcement ratio), Timeout (during timeout period), Inactive (non-reinforced lever).\n\nTimeout mode matters most when the reinforcement ratio is greater than 1. On \"Every active press\" (the default, and how every previous version behaved) any active press opens the timeout window — and presses landing inside that window are logged as Timeout presses, which do NOT count toward the ratio. With FR2 and a 20 s timeout, the animal therefore has to space every press by at least 20 s to earn anything; on VI it can also miss an availability window outright. Choose \"Reward-triggering press only\" to have the timeout start just on the press that completes the ratio and fires the reward, so intermediate presses accumulate freely.\n\nNote: firmware keeps a single timeout mode for both levers, exactly as it does for the timeout duration — setting it on one lever sets it for both. The control is only available on FR, PR and VI; Omission and Pavlovian have no lever timeout.\n\nNote: the reinforcement ratio itself is not set here. It is one scheduler-wide value — shared by both levers, not per-lever — configured once under Paradigm Settings.",
          },
          {
